class Solution:
    def threeSum(self, nums: List[int]) -> List[List[int]]:
        ...
        #because it is sorted we want to make sure that the outputted vals are not repeated so you want to setup a loop that checks for repeat value, if repeated only update one pointer. (only need to update one pointer because it is sorted)
